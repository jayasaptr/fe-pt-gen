...
const CategoryBarang = () => {
    ...
    const validation: any = useFormik({
        ...
        onSubmit: (values) => {
            if (isEdit) {
                ...
                handlePostCategoryBarang(values);
            }
            if (isLoading) {
                toggle();
            }
    ...
    const fetchDataBarang = async () => {
        setLoadingV(true);
        try {
            const userResponse = await axiosInstance.get("/product-categories", {
                headers: {
    ...
    const handlePostCategoryBarang = async (data: any) => {
        try {
            setIsLoading(true);
            const formData = new FormData();
            formData.append("name", data.name);

            const userResponse = await axiosInstance.post("/product-categories", formData, {
                headers: {
                    Authorization: `Bearer ${user.data.token}`,
            ...
            if (userResponse.data.status === true) {
                Success("Data Category Barang Berhasil Ditambahkan");
                fetchDataBarang();
                toggle();
            }
        } catch (error: any) {
            Error("Data Category Barang Gagal Ditambahkan");
            if (error.response.status === 401) {
                localStorage.removeItem("authUser");
                naviagate("/login");
            }
    ...
    const handleUpdateSuratMasuk = async (data: any) => {
        try {
            ...
            if (userResponse.data.status === true) {
                Success("Data Category Barang Berhasil Diupdate");
                fetchDataBarang();
                toggle();
            }
        } catch (error: any) {
    ...
    const handleDeleteSuratMasuk = async (id: any) => {
        try {
            setIsLoading(true);
            const userResponse = await axiosInstance.delete(`/product-categories/${id}`, {
                headers: { Authorization: `Bearer ${user.data.token}` },
            });

            if (userResponse.data.status === true) {
                Success("Data Category Barang Berhasil Dihapus");
                fetchDataBarang();
            }
        } catch (error: any) {
    ...
    return (
        <>
            <BreadCrumb title="Category Barang" pageTitle="Category Barang" />
            <DeleteModal
                show={deleteModal}
                onHide={deleteToggle}
                ...
                <div className="card-body">
                    <div className="flex items-center gap-3 mb-4">
                        <h6 className="text-15 grow">
                            Category Barang (<b className="total-Employs">{data.length}</b>)
                        </h6>
                        <div className="shrink-0">
                            ...
                            >
                                ...
                                <span className="align-middle">Add Category Barang</span>
                            </Link>
                        </div>
                    </div>
                    {data && data.length > 0 ? (
            ...
            >
                ...
                >
                    <Modal.Title className="text-16">
                        {!!isEdit ? "Edit Category Barang" : "Add Category Barang"}
                    </Modal.Title>
                </Modal.Header>
