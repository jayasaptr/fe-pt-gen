...
const PurchaseItemPage = () => {
    ...
    const { id: purchase_id } = useParams<{ id: string }>();

    const fetchDataPurchase = async () => {
        setLoadingV(true);
        try {

            const userResponse = await axiosInstance.get("/purchase-items", {
                headers: {
    ...
    const handlePostPurchase = async (data: any) => {
        try {
            ...
            const userResponse = await axiosInstance.post("/purchase-items", formData, {
                headers: {
                    Authorization: `Bearer ${user.data.token}`,
    ...
    const handleDeleteDataPurchase = async (id: any) => {
        try {
            setIsLoading(true);
            const userResponse = await axiosInstance.delete(`/purchase-items/${id}`, {
                headers: {
                    Authorization: `Bearer ${user.data.token}`,
                },
            });
    ...
    return (
        <>
            <BreadCrumb title="Data Purchase Item" pageTitle="Data Purchase" />
            <DeleteModal
                show={deleteModal}
                onHide={deleteToggle}
                ...
                <div className="card-body">
                    <div className="flex items-center gap-3 mb-4">
                        <h6 className="text-15 grow">
                            Purchase Item (<b className="total-Employs">{data.length}</b>)
                        </h6>
                        <div className="shrink-0">
                            ...
                            >
                                ...
                                <span className="align-middle">Add Purchase Item</span>
                            </Link>
                        </div>
                    </div>
                    {data && data.length > 0 ? (
